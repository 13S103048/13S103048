...
char  g_szLocalAddr[MAX_ADDR_LEN]; //本地地址
 BOOL  g_bLoggedIn;

UINT FtpServerThread(LPVOID param)
{
	cout<<"this is ftp server"<<endl;
	WSADATA wsaData;     //WSA启动接收返回
   SOCKET sListen, sAccept; // tcp
   SOCKET sUDP; //udp  
   SOCKADDR_IN inetAddr;  // sockaddr_in结构
   SOCKADDR_IN inetAddrUDP; 
   DWORD dwFlags;
   DWORD dwThreadId;
   DWORD dwRecvBytes;
   INT   nRet;
   ...
   if (((sListen = WSASocket(AF_INET, SOCK_STREAM, 0, NULL, 0, 
      WSA_FLAG_OVERLAPPED)) == INVALID_SOCKET)||((sUDP = WSASocket(AF_INET, SOCK_DGRAM, 0, NULL, 0, 
   ...
   inetAddr.sin_family = AF_INET;//socket编程只能用地址
   inetAddr.sin_addr.s_addr = htonl(INADDR_ANY);  //存储IP地址
   inetAddr.sin_port = htons(FTP_PORT);//端口号 
   
   inetAddrUDP.sin_family = AF_INET;//UDP
   inetAddrUDP.sin_addr.s_addr = htonl(INADDR_ANY);  
   inetAddrUDP.sin_port = htons(UDP_FTP_PORT);//端口号; 

   if ((bind(sListen, (PSOCKADDR) &inetAddr, sizeof(inetAddr)) == SOCKET_ERROR)||(bind(sUDP, (PSOCKADDR) &inetAddrUDP, sizeof(inetAddrUDP)) == SOCKET_ERROR))
   {
   ...
   printf("******Ftp Server is Starting now! *******\n");
...
int	DataConn( SOCKET& s, DWORD dwIp, WORD wPort, int nMode ) 
{
	...
	{ 
		inetAddr.sin_port = htons( DATA_FTP_PORT );//端口20
